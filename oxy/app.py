...
def main():
    ...
    parser.add_argument("--theme", help="override theme (builtin: minimal, cyber, aurora; or ~/.oxy/themes/<name>.json)")
    ...
    parser.add_argument("--lang", choices=["en", "fa", "zh"], help="override interface language for this session")
    ...
    if args.resume:
        ...
        if resumed_sess:
            ...
            render_info(_t("resumed_session", session_id=resumed_sess.session_id, count=len(engine.history)), theme)
        else:
    ...
    if engine.session.has_unresolved_tool_calls():
        ...
        render_error(_t("orphaned_tool_calls", count=len(orphaned)), theme)
        for tc in orphaned:
            fn = tc.get("function", {})
        ...
        render_info(_t("discarded_orphans", count=discarded), theme)
    ...
    while True:
        ...
        if user_input.startswith("/"):
            ...
            if new_theme:
                theme = new_theme
                try:
                    engine.set_theme(config.get("theme", "minimal"))
                except Exception:
                    pass
                session = make_prompt_session(theme)
            if not keep_running:
        ...
        try:
            try:
                ...
                from .i18n import t as _t
                render_error_panel(f"{_t('agent_error')} ({type(e).__name__}): {e}", theme)
                render_info(_t("history_preserved", count=len(engine.history)), theme)
        finally:
